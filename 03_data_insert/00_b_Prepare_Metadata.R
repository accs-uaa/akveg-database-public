...
sql_metadata = path(output_folder, 
                    '00_b_Insert_Metadata.sql')

# Identify metadata tables
dictionary_file = path(data_folder,
                      'database_dictionary.xlsx')
schema_file = path(data_folder,
                    'database_schema.xlsx')
organization_file = path(data_folder,
                          'organization.xlsx')
...
# Export tables
field_list = c('completion', 'cover_method', 'cover_type', 'crown_class',
...
table_list = list(completion_table, cover_method_table, cover_type_table, crown_class_table,
...
statement = c(
  statement,
  'INSERT INTO ground_element (ground_element_code, ground_element) VALUES')
