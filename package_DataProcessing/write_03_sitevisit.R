# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Create template file for Site Visit table
# Author: Amanda Droghini, Alaska Center for Conservation Science
# Last Updated: 2024-07-26
# Usage: Should be executed in R 4.4.1+.
# Description: "Create template file for Site Visit table" reads in a .R script file and replaces values as specified in a CSV configuration file. The output is a .R script file with appropriate file paths and project code values that can then be modified for data ingestion needs.
...
# Define input files
template_file = file.path(repository_folder, "package_DataProcessing/template_03_sitevisit.R")
...
site_visit = parameters[which(parameters$parameter == 'site_visit_path'), 'value']

# Define replacement values
project_folder_path = file.path(data_folder, project_name)
source_folder = file.path(project_folder_path, "source")
site_visit_path = file.path(source_folder, site_visit)
...
template_txt = gsub(pattern = "plot_folder_path", 
             ...
             x = template_txt)
template_txt = gsub(pattern = "site_visit_path", 
             replacement = paste0("'", site_visit_path, "'"),
             x = template_txt)

# If needed, replace read_csv with read_xlsx
if (endsWith(site_visit_path,"xlsx")) {
  message("Source file format XLSX")
  template_txt = gsub(pattern = "site_visit_original = read_csv",
               replacement = "site_visit_original = read_xlsx",
               x = template_txt)
} else {
...
# Define output
output_file = paste(project_name, "03_sitevisit.R", sep = "_")
