# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Parse FWS Selawik 2005 upload from schema 1.0 to 2.0
# Author: Timm Nawrocki, Alaska Center for Conservation Science
# Last Updated: 2022-10-13
# Usage: Script should be executed in R 4.0.0+.
# Description: "Parse FWS Selawik 2005 data from schema 1.0 to 2.0" updates the data from the 1.0 to 2.0 AKVEG Database schema format. New fields are added as necessary to match VTWG Minimum Standards per data tier and NODATA values are enforced.
# ---------------------------------------------------------------------------
...
# Define project
project_folder = '09_fws_selawik_2005'

# Define input data files
site_input = 'usfwsSelawikTalbot_2005_Site.xlsx'
cover_input = 'usfwsSelawikTalbot_2005_Cover.xlsx'

# Define output data files
site_output = '02_site_fwsselawik2005'
site_visit_output = '03_sitevisit_fwsselawik2005'
cover_output = '05_vegetationcover_fwsselawik2005'
...
site_data = site_original %>%
  ...
  mutate(establishing_project = 'fws_selawik_2005') %>%
  # Select final fields
...
site_visit_data = cover_original %>%
  ...
  mutate(scope_vascular = 'non-trace species') %>%
  mutate(scope_bryophyte = 'non-trace species') %>%
  mutate(scope_lichen = 'non-trace species') %>%
