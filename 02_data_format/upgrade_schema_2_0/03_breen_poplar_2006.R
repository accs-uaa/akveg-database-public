# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# Parse Breen Poplar 2006 upload from schema 1.0 to 2.0
# Author: Timm Nawrocki, Alaska Center for Conservation Science
# Last Updated: 2022-10-13
# Usage: Script should be executed in R 4.0.0+.
# Description: "Parse Breen Poplar 2006 data from schema 1.0 to 2.0" updates the data from the 1.0 to 2.0 AKVEG Database schema format. New fields are added as necessary to match VTWG Minimum Standards per data tier and NODATA values are enforced.
# ---------------------------------------------------------------------------
...
# Define project
project_folder = '03_breen_poplar_2006'

# Define input data files
site_input = 'poplarBreen_2006_Site.xlsx'
cover_input = 'poplarBreen_2006_Cover.xlsx'

# Define output data files
site_output = '02_site_breenpoplar2006'
site_visit_output = '03_sitevisit_breenpoplar2006'
cover_output = '05_vegetationcover_breenpoplar2006'
...
site_data = site_original %>%
  ...
  mutate(establishing_project = 'breen_poplar_2006') %>%
  # Select final fields
...
site_visit_data = cover_original %>%
  ...
  mutate(env_observer = 'Amy Breen') %>%
  mutate(soils_observer = 'NULL') %>%
  # Correct project code
  mutate(project_code = 'breen_poplar_2006') %>%
