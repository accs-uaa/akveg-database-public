# -*- coding: utf-8 -*-
# ---------------------------------------------------------------------------
# "Format Vegetation Cover for Yukon Biophysical Inventory Data System Plots"
# Author: Amanda Droghini, Alaska Center for Conservation Science
# Last Updated: 2025-06-19
# Usage: Must be executed in R version 4.5.1+.
# Description: "Format Vegetation Cover for Yukon Biophysical Inventory Data System Plots" formats vegetation cover data for ingestion into the AKVEG Database. The script appends unique site visit identifiers, corrects taxonomic names using the AKVEG comprehensive checklist, performs QA/QC checks, and enforces formatting to match the AKVEG template. The output is a CSV table that can be converted and included in a SQL INSERT statement.
# ---------------------------------------------------------------------------
...
veg_data = veg_original %>%
  ...
  select(site_visit_code, Scientific.name, Veg.cover.pct, 
...
veg_taxa = veg_data %>% 
  mutate(name_original = str_remove(Scientific.name, " sp\\."),
...
veg_taxa = veg_taxa %>% 
  mutate(name_adjudicated = case_when(grepl("^Alopecurus", Scientific.name) & is.na(name_adjudicated) ~ "Alpinus",
                                      Scientific.name == 'Artemisia canadensis' ~ "Artemisia borealis",
                                      Scientific.name == 'Artemisia norvegica' ~ "Artemisia arctica",
                                      Scientific.name == 'Artemisia rupestris' ~ "Artemisia woodii",
                                      Scientific.name == 'Astragalus adsurgens' ~ 'Astragalus laxmannii',
                                      Scientific.name == 'Betula x dugleana' ~ "Betula cf. occidentalis",
                                      Scientific.name == 'Bromus carinatus' ~ 'grass (Poaceaea)',
                                      Scientific.name == 'Cardamine oligosperma' ~ 'Cardamine umbellata',
                                      Scientific.name == 'Carex x flavicans' ~ 'Carex subspathacea',
                                      Scientific.name == 'Cryptogramma crispa var.' ~ 'Cryptogramma acrostichoides',
                                      Scientific.name == 'Draba alpina' ~ "Draba",
                                      Scientific.name == 'Dryas octopetala' ~ 'Dryas ajanensis ssp. beringensis',
                                      Scientific.name == 'Galium labradoricum' ~ 'Galium',
                                      Scientific.name == 'Lagotis glauca ssp. minor' | Scientific.name == 'Lagotis stelleri' ~ 'Lagotis glauca ssp. lanceolata',
                                      Scientific.name == 'Melandrium apetalum' ~ 'Silene uralensis',
                                      grepl("^Minuartia", Scientific.name) & is.na(name_adjudicated) ~ "forb",
                                      Scientific.name == 'Pedicularis sudetica' ~ 'Pedicularis',
                                      Scientific.name == 'Picea glauca x mariana' ~ 'Picea',
                                      Scientific.name == 'Saxifraga bronchialis' ~ 'Saxifraga funstonii',
                                      Scientific.name == 'Saxifraga davurica' ~ 'Micranthes',
                                      Scientific.name == 'Smelowskia calycina (split)' ~ 'Smelowskia',
                                      Scientific.name == 'Taraxacum lyratum' ~ 'Taraxacum scopulorum',
                                      Scientific.name == 'Xanthoparmelia chlorochroa' ~ 'Xanthoparmelia',
...
veg_final = veg_final %>% 
  mutate(cover_type = "absolute foliar cover", # Review
